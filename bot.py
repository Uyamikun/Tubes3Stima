...
def readFile(filename,array):
    f = open(os.path.join(SETTINGS_DIR, "database/"+ filename) , 'r')
    for item in f:
...
def checkTask(s):
    ...
    for i in arrayTugas:
        ...
        #print(perkata)
        if(perkata[1] == matkul):
            itemsplit1 = perkata[0].split("/")
            makeTime1 = date(int("20" + itemsplit1[2]), int(itemsplit1[1]),int(itemsplit1[0]))
            if(tugas != -1 and perkata[2][0:2].lower() == "tu" and makeTime1 >= cdate):
                return perkata[0]
            elif(makalah != -1 and perkata[2].lower() == "makalah" and makeTime1 >= cdate):
                return perkata[0]
